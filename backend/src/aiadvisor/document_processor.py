...
class DocumentParser:
    ...
    async def _parse_pdf(self, file_path: Path) -> tuple[str, Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                raise AIAdvisorException(f"PDF 파싱 실패: {str(e)}, 대체 방법도 실패: {str(fallback_error)}")
    ...
    async def _create_chunks(self, content: str, metadata: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for chunk in chunks:
            # 최소 길이 및 의미있는 내용 확인
            if len(chunk.strip()) >= 50 and any(c.isalnum() or ord(c) > 127 for c in chunk):
    ...
    def _calculate_chunk_quality(self, chunk: str) -> float:
        ...
        try:
            ...
            # 기본 길이 점수
            length_score = min(len(chunk) / 300, 1.0)  # 300자 기준
            score += length_score * 0.3
            
            # 한글 비율
            korean_chars = sum(1 for c in chunk if ord(c) >= 0xAC00 and ord(c) <= 0xD7A3)
            korean_ratio = korean_chars / len(chunk) if chunk else 0
            score += korean_ratio * 0.4
            
            # 문장 구조 (마침표, 줄바꿈 등)
            sentence_markers = chunk.count('.') + chunk.count('。') + chunk.count('!') + chunk.count('?')
            structure_score = min(sentence_markers / 3, 1.0)
            score += structure_score * 0.2
            
            # 숫자와 영문 혼재 (기술 문서에서 중요)
            mixed_content = any(c.isdigit() for c in chunk) and any(c.isalpha() for c in chunk)
            if mixed_content:
                score += 0.1
...
class DocumentProcessor:
        ...
        try:
            ...
            try:
                ...
                processed_result = {
                    ...
                    "extraction": extraction_result,
                    "processing_status": "completed",
                    "sections": parsing_result.get("chunks", []),
    ...
    def _generate_document_summary(self, doc_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            chunks = parsing_info.get('chunks', sections)  # parsing.chunks 우선, 없으면 sections 사용
            
            if not chunks:
                # 청크가 없는 경우 기본 정보만
                return f"[{category}] {filename} ({self._format_file_size(file_size)}) - 문서 내용을 처리하지 못했습니다."
            
            # 3. 청크에서 내용 추출
            content_parts = []
            total_chars = 0
            max_chars = 800  # 요약용 최대 문자 수
            
            for i, chunk in enumerate(chunks[:10]):  # 최대 10개 청크
                content = chunk.get('content', '').strip()
                if content:
                    # 청크 내용이 너무 길면 첫 부분만 추출
                    if len(content) > 200:
                        content = content[:200] + "..."
            ...
            if not content_parts:
                return f"[{category}] {filename} ({self._format_file_size(file_size)}) - 내용을 추출할 수 없습니다."
            ...
            if len(combined_content) > 400:
                ...
                first_chunk = content_parts[0] if content_parts else ""
                last_chunk = content_parts[-1] if len(content_parts) > 1 else ""
                
                if first_chunk and last_chunk and first_chunk != last_chunk:
                    summary_content = f"{first_chunk} ... {last_chunk}"
                else:
                    summary_content = first_chunk
            ...
            summary = f"[{category}] {filename} ({self._format_file_size(file_size)}, {chunk_count}개 섹션) - {summary_content}"
