...
class LotTrackingEngine(ProcessManagementEngine):
    ...
    def load_lot_data(self) -> Tuple[List[Dict], bool]:
        """Lot 데이터 로드"""
        production_files = self.get_production_data_files()
        
        if not production_files:
            return self.generate_sample_lot_data(), True  # 샘플 데이터 사용
        ...
        try:
            ...
            if all_lots:
                return all_lots, False  # 실제 데이터 사용
            else:
                return self.generate_sample_lot_data(), True  # 샘플 데이터 사용
...
class EquipmentMonitoringEngine(ProcessManagementEngine):
    ...
    def calculate_equipment_status(self, lots: List[Dict], is_sample_data: bool = False) -> Tuple[List[Dict], bool]:
        """설비별 상태 계산"""
        equipment_data = {}
        ...
        return sorted(equipment_status, key=lambda x: x["ID"]), is_sample_data
    
    def calculate_equipment_detailed_stats(self, lots: List[Dict], date_filter: str = "전체", 
                                         start_date: Optional[datetime] = None, 
                                         end_date: Optional[datetime] = None) -> Tuple[Dict[str, Dict], bool]:
        """설비별 상세 통계 계산"""
        equipment_stats = {}
        
        # 실제 생산 데이터에서 원시 데이터 로드
        raw_data = self.load_raw_production_data()
        is_sample_data = len(raw_data) == 0
        
        # 날짜 필터링 적용
        if date_filter != "전체" and not is_sample_data:
        ...
        return final_stats, is_sample_data
...
class AnomalyDetectionEngine(ProcessManagementEngine):
    ...
    def detect_anomalies(self, lots: List[Dict], temp_threshold: float, 
                        ...
                        start_date: Optional[datetime] = None, 
                        end_date: Optional[datetime] = None) -> Tuple[List[Dict], bool]:
        """실제 생산 데이터에서 이상탐지 수행"""
        raw_data = self.load_raw_production_data()
        is_sample_data = len(raw_data) == 0  # 실제 데이터가 없으면 샘플 데이터 사용
        
        # 실제 데이터가 없으면 샘플 이상탐지 데이터 생성
        if is_sample_data:
            sample_alerts = []
            np.random.seed(42)
            ...
            # 몇 가지 샘플 이상 상황 생성
            sample_scenarios = [
                {"lot_id": "LOT-2024-003", "설비": "EQ-05", "내용": f"온도 {210.5}°C (임계값: {temp_threshold}°C)", "상태": "주의"},
                {"lot_id": "LOT-2024-007", "설비": "EQ-04", "내용": f"압력 {3.8} bar (임계값: {pressure_threshold} bar)", "상태": "경고"},
                {"lot_id": "LOT-2024-015", "설비": "EQ-10", "내용": f"순도 {92.3}% (최소값: {purity_threshold}%)", "상태": "주의"}
            ]
            
            # 임계값에 따라 동적으로 조정
            for scenario in sample_scenarios:
                if ("온도" in scenario["내용"] and temp_threshold < 210) or \
                   ("압력" in scenario["내용"] and pressure_threshold < 3.8) or \
                   ("순도" in scenario["내용"] and purity_threshold > 92.3):
                    sample_alerts.append(scenario)
            
            return sample_alerts, True
        ...
        return anomaly_alerts[:10], False  # 최대 10개만 반환, 실제 데이터 사용
    
    def calculate_anomaly_statistics(self, lots: List[Dict], temp_threshold: float, 
                                   pressure_threshold: float, purity_threshold: float, 
                                   yield_threshold: float, date_filter: str = "전체",
                                   start_date: Optional[datetime] = None, 
                                   end_date: Optional[datetime] = None) -> Tuple[Dict[str, Any], bool]:
        """이상탐지 통계 계산"""
        raw_data = self.load_raw_production_data()
        is_sample_data = len(raw_data) == 0
        
        # 실제 데이터가 없으면 샘플 통계 반환
        if is_sample_data:
            sample_stats = {
                "total_lots": 46,  # 샘플 Lot 총 개수
                "anomaly_count": 3,
                "anomaly_rate": 6.5,
                "temp_anomalies": 1,
