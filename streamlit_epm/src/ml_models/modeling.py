...
def train_model(X_train, y_train, model_type, X_test=None, y_test=None, **params):
    ...
    Args:
        X_train: 훈련 데이터 특성
        y_train: 훈련 데이터 타겟
        model_type: 모델 타입 (linear, ridge, lasso, random_forest, gradient_boosting, svr, catboost, xgboost)
        X_test: 테스트 데이터 특성 (선택사항)
        y_test: 테스트 데이터 타겟 (선택사항)
        **params: 모델별 하이퍼파라미터
    ...
    if model_type == "linear":
        ...
        if X_test is not None and y_test is not None:
            ...
            model = xgb.train(
                ...
                verbose_eval=params.get('verbose', 0)  # Print info
            )
...
def plot_feature_importance(model, feature_names, model_type):
    ...
    Returns:
        Plotly 그래프 객체, 피처 중요도 DataFrame
    """
    fig = go.Figure()
