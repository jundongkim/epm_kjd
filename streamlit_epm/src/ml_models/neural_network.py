...
class NeuralNetworkModel:
    ...
    def train(self, df: pd.DataFrame, target_col: str, 
              ...
              max_iter: int = 200, test_size: float = 0.2, 
              random_state: int = 42, callback=None) -> Dict[str, Any]:
        """
        모델 훈련
        ...
        if hasattr(self.model, 'loss_curve_') and self.model.loss_curve_ is not None:
            training_history = {
                'loss': self.model.loss_curve_.tolist(),
                'iterations': list(range(1, len(self.model.loss_curve_) + 1))
            }
