...
def parse_arguments():
    ...
    parser.add_argument('--model_dir', type=str, 
                        help='모델 저장 디렉토리 (기본값: "models_xgb")')
    parser.add_argument('--plot_dir', type=str, 
                        help='시각화 저장 디렉토리 (기본값: "plots_xgb")')
...
def train_model(X_train, y_train, X_test, y_test, plot_dir, model_dir, n_estimators=1000, learning_rate=0.01, max_depth=8, random_state=42):
    """
    XGBoost 모델을 학습시킵니다.
    """
    ...
    # 학습 결과 정보 추출
    train_losses = results['train']['mae']
    test_losses = results['validation']['mae']
    ...
    # 차트 저장
    fig.write_html(f"{plot_dir}/xgb_core_training_history.html")
    
    # 모델 저장
    # JSON 형식으로 저장
    model_json_path = f"{model_dir}/xgboost_booster.json"
    model.save_model(model_json_path)
    
    # Pickle 형식으로도 저장
    model_pkl_path = f"{model_dir}/xgboost_booster.pkl"
    with open(model_pkl_path, 'wb') as f:
        pickle.dump(model, f)
    
    print(f"XGBoost 모델 학습 완료. 최적 반복 횟수: {model.best_iteration}")
    print(f"모델이 저장되었습니다:")
    print(f"  - JSON 형식: {model_json_path}")
    print(f"  - Pickle 형식: {model_pkl_path}")
...
def evaluate_model(model, X_test, y_test, plot_dir):
    ...
    fig.update_layout(
        title=dict(
            text=f'XGBoost 예측값 vs 실제값 (RMSE: {rmse:.4f}, MAE: {mae:.4f}, MAPE: {mape:.2f}%, R²: {r2:.4f})',
            font=dict(color=ECOPRO_COLORS['dark_blue'], size=20)
        ),
        xaxis_title='실제값',
        yaxis_title='예측값',
    ...
    # 차트 저장
    fig.write_html(f"{plot_dir}/xgb_core_prediction_vs_actual.html")
    ...
    # 차트 저장
    fig_res.write_html(f"{plot_dir}/xgb_core_residual_analysis.html")
    
    return rmse, mae, r2, y_pred, residuals

def analyze_feature_importance(model, X_train, column_info, plot_dir):
    """
    모델의 특성 중요도를 분석합니다.
    """
    print("XGBoost 특성 중요도 분석 중...")
    
    # 특성 중요도 계산 (기본 gain 방식)
    ...
    # 일부 특성이 누락될 수 있으므로, 모든 특성을 포함하도록 수정
    all_features = X_train.columns.tolist()
    all_scores = []
    
    for feature in all_features:
        if feature in importance_scores:
            all_scores.append(importance_scores[feature])
        else:
            all_scores.append(0)
    ...
    fig.update_layout(
        title=dict(text=f'XGBoost 상위 {top_n}개 특성 중요도', font=dict(color=ECOPRO_COLORS['dark_blue'], size=20)), 
        xaxis_title='중요도 점수', 
        yaxis_title='특성', 
        template='plotly_white', 
        height=600, 
    ...
    # 차트 저장
    fig.write_html(f"{plot_dir}/xgb_core_feature_importance.html")
    ...
    # 차트 저장
    fig_types.write_html(f"{plot_dir}/xgb_core_feature_type_importance.html")
    
    return importance_df, type_importance

def generate_shap_analysis(model, X_test, plot_dir):
    """
    XGBoost 모델에 대한 SHAP 분석을 수행합니다.
    """
    print("\n--- SHAP 분석 시작 ---")
    ...
    try:
        ...
        # DMatrix로 변환 (XGBoost에서 필요)
        dmatrix_sample = xgb.DMatrix(X_sample)
        
        # SHAP 값 계산 (XGBoost의 경우 DMatrix 사용)
        shap_values = explainer.shap_values(X_sample)
        
        # XGBoost도 때로 리스트 형태의 SHAP 값을 반환할 수 있으므로 확인
        if isinstance(shap_values, list):
            # 단일 출력 회귀 문제에서는 첫 번째 요소만 사용
            print("리스트 형태의 SHAP 값 감지, 첫 번째 요소만 사용합니다.")
        ...
        # SHAP 값 저장
        print(f"SHAP 값 형태: {shap_values.shape}, 특성 수: {len(feature_names)}")
        
        try:
            # SHAP 값을 DataFrame으로 변환하여 CSV 저장
            shap_df = pd.DataFrame(shap_values, columns=feature_names)
            shap_df['prediction'] = model.predict(dmatrix_sample)
            
            # CSV 파일 경로 및 디렉토리 확인
            if not os.path.exists(plot_dir):
                os.makedirs(plot_dir, exist_ok=True)
                print(f"디렉토리 생성: {plot_dir}")
                
            shap_csv_path = os.path.join(plot_dir, "xgb_shap_values.csv")
            print(f"SHAP 값을 CSV 파일로 저장 시도: {shap_csv_path}")
            
            # CSV 파일로 저장
            shap_df.to_csv(shap_csv_path, index=False)
            print(f"SHAP 값이 CSV 파일로 저장되었습니다: {shap_csv_path}")
        except Exception as csv_err:
            print(f"CSV 파일 저장 중 오류 발생: {csv_err}")
        ...
        # HTML 저장
        shap_importance_html_path = os.path.join(plot_dir, "xgb_shap_feature_importance.html")
        ...
        # HTML 저장
        summary_html_path = os.path.join(plot_dir, "xgb_shap_summary_enhanced.html")
        fig_summary.write_html(summary_html_path)
        
        # 3. 기존 SHAP 요약 시각화 (특성-값 관계)
        shap_summary_data = []
        ...
        # HTML 저장
        shap_summary_html_path = os.path.join(plot_dir, "xgb_shap_summary.html")
        fig_shap_summary.write_html(shap_summary_html_path)
        
        # 4. SHAP 의존성 플롯 (상위 특성들에 대해)
        # 상위 5개 특성에 대한 의존성 플롯 생성
        ...
        for feature in top20_features:
            ...
            # HTML 저장
            dep_html_path = os.path.join(plot_dir, f"xgb_shap_dependence_{feature}.html")
        ...
        # HTML 저장
        force_html_path = os.path.join(plot_dir, "xgb_shap_force_plot.html")
        ...
        if len(top5_features) >= 2:
            ...
            # HTML 저장
            interaction_html_path = os.path.join(plot_dir, f"xgb_shap_interaction_{feature1}_{feature2}.html")
            fig_interaction.write_html(interaction_html_path)
        
        print(f"SHAP 분석 결과 저장 완료: {plot_dir}")
        print(f"생성된 SHAP 시각화 파일:")
        print(f"  - 특성 중요도: xgb_shap_feature_importance.html")
        print(f"  - SHAP 요약 플롯 (향상된): xgb_shap_summary_enhanced.html")
        print(f"  - SHAP 요약 플롯 (기본): xgb_shap_summary.html")
        print(f"  - 의존성 플롯: xgb_shap_dependence_*.html")
        print(f"  - Force 플롯: xgb_shap_force_plot.html")
        if len(top5_features) >= 2:
            print(f"  - 상호작용 플롯: xgb_shap_interaction_{top5_features[0]}_{top5_features[1]}.html")
...
def main():
    ...
    for data_file in data_files:
        ...
        plot_dir = args.plot_dir if args.plot_dir else f"plots_xgb{suffix}"
        model_dir = args.model_dir if args.model_dir else f"models_xgb{suffix}"
        ...
        # 데이터 준비
        X_train, X_test, y_train, y_test, column_info = prepare_data(df)
        ...
        rmse, mae, r2, y_pred, residuals = metrics
        
        # 특성 중요도 분석
        importance_df, type_importance = analyze_feature_importance(
            model, X_train, column_info, plot_dir=plot_dir
        )
        
        # SHAP 분석 수행
        try:
            print(f"\n=== {file_name} 데이터셋에 대한 SHAP 분석 시작 ===")
            
            # Plot 디렉토리 확인 및 생성
            if not os.path.exists(plot_dir):
                os.makedirs(plot_dir, exist_ok=True)
                print(f"SHAP 분석용 디렉토리 생성됨: {plot_dir}")
            
            # SHAP 분석 실행
            shap_importance = generate_shap_analysis(model, X_test, plot_dir=plot_dir)
            
            if shap_importance is not None:
                print(f"SHAP 분석 성공적으로 완료: {len(shap_importance)} 특성의 중요도 계산됨")
            else:
                print("SHAP 분석은 완료되었으나 결과가 None입니다. generate_shap_analysis 함수 내부를 확인하세요.")
        ...
        # 결과를 JSON 파일로 저장
        results_file_path = os.path.join(model_dir, 'xgboost_results.json')
        try:
            with open(results_file_path, 'w', encoding='utf-8') as f:
                json.dump(results_data, f, ensure_ascii=False, indent=4)
            print(f"XGBoost 모델 결과 요약 정보가 {results_file_path}에 저장되었습니다.")
        except Exception as e:
    ...
    # CatBoost 모델에 대한 안내 메시지 추가
    print("\n--- CatBoost 모델 학습 방법 ---")
    if args.data_file:
        print(f"다음 명령을 실행하여 '{args.data_file}' 파일에 대한 CatBoost 모델을 학습할 수 있습니다:")
        print(f"python 2_training_catboost.py --data_file {args.data_file}")
    else:
        print("다음 명령을 실행하여 모든 normalized_data*.csv 파일에 대한 CatBoost 모델을 학습할 수 있습니다:")
        print("python 2_training_catboost.py")
    print("특정 모델 및 시각화 저장 디렉토리 지정 예시:")
    print("python 2_training_catboost.py --data_file data_file.csv --model_dir models_cbm_custom --plot_dir plots_cbm_custom")
    print("----------------------------")
