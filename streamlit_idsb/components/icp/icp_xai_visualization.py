...
def show_icp_xai_visualization():
    """
    ICP XAI(설명 가능한 AI) 시각화를 보여주는 메인 함수
    """
    # AI 분석을 위한 세션 상태 초기화
    init_session_state(key_prefix="icp_xai_visualization")
    
    st.title("ICP 설명 가능한 AI (XAI) 분석")
    ...
    # 시각화 카테고리 분류
    categories = {
        "특성 중요도": ["shap_feature_importance", "feature_importance"],
        "SHAP 요약": ["shap_summary"],
        "SHAP 의존성": ["shap_dependence_"],
        "SHAP Force 플롯": ["shap_force_plot"],
        "SHAP 상호작용": ["shap_interaction_"]
    }
    ...
    # 프롬프트 생성
    prompt = f"""
    다음은 ICP 데이터의 XAI(설명 가능한 AI) 시각화입니다:
    ...
    try:
        is_running = display_analysis_ui(
            ...
            key_prefix="icp_xai_visualization",
...
def show_shap_data():
    ...
    shap_csv = os.path.join(PLOTS_DIR, "shap_values.csv")
    ...
    if os.path.exists(shap_csv):
        ...
        with col2:
            st.metric("특성 수", df.shape[1] - 2)  # prediction과 actual 열을 제외
        ...
        if feature_cols:
            ...
            if search_term:
                ...
                if filtered_cols:
                    ...
                    st.dataframe(df[filtered_cols + ['prediction', 'actual']].head(20))
                else:
        ...
        if download_option == "선택한 특성만" and feature_cols:
            ...
            if selected_cols:
                ...
                download_cols = selected_cols + ['prediction', 'actual']
        ...
        # 프롬프트 생성
        prompt = """
        다음은 XGBoost 모델의 원시 SHAP 값 데이터에 대한 분석입니다:
